...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    async def async_update_data():
        ...
        if not app or 'result' not in app:
            ...
            offline_state = hass.localize(f"component.{DOMAIN}.state.kodi_offline")
            return {
        ...
        if not players or not players.get('result'):
            no_playback_state = hass.localize(f"component.{DOMAIN}.state.no_playback")
            no_audio_state = hass.localize(f"component.{DOMAIN}.state.no_audio_info")
        ...
        if audio_data and 'result' in audio_data and audio_data['result'].get('audiostreams'):
            ...
            if current is not None and current < len(streams):
                ...
                channel_str_map = {
                    2: hass.localize(f"component.{DOMAIN}.state.stereo"),
                    6: hass.localize(f"component.{DOMAIN}.state.channel_5_1"),
                    8: hass.localize(f"component.{DOMAIN}.state.channel_7_1")
                }
                channel_str = channel_str_map.get(channels, hass.localize(f"component.{DOMAIN}.state.channel_multi", {"channels": str(channels)}))
